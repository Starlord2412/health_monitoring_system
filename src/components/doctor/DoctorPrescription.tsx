...
const nameToId: Record<string, string> = {
  "John Doe": "PT-001",
  "Jane Smith": "PT-002",
  "Robert Wilson": "PT-003",
  "Emily Davis": "PT-004",
  "Michael Brown": "PT-005",
  "Sarah Johnson": "PT-006",
};
...
export function DoctorPrescription() {
  ...
  // Load prescriptions from Realtime Database
  useEffect(() => {
    ...
    const unsub = onValue(
      ...
      (snapshot) => {
        ...
        const list: Prescription[] = Object.values(data).map((item: any) => ({
          id: item.id,
  ...
  const handleSave = async () => {
    if (!form.patient || !form.medication || !form.dosage) return;

    const dateIssued = new Date().toISOString().split("T")[0];
    const base = {
      patientId: nameToId[form.patient] ?? "",
    ...
    if (editingId == null) {
      ...
      const nextId = prescriptions.length
        ? Math.max(...prescriptions.map((p) => p.id)) + 1
        : 1;
      const payload = { ...base, id: nextId };

      const newRef = push(prescriptionsRef);
      await set(newRef, payload);
    } else {
      // update existing prescription
      const key = String(editingId); // keys "1","2","3",... in DB
      const payload = { ...base, id: editingId };
      const itemRef = ref(db, `prescriptions/${key}`);
            ...
            <DialogContent className="max-w-2xl rounded-2xl border border-slate-200 bg-white text-slate-900 shadow-xl">
              ...
              <div className="mt-4 space-y-4">
                ...
                <Button
                  ...
                  onClick={handleSave}
                >
                  {editingId == null ? "Add prescription" : "Save changes"}
                </Button>
              </div>
            </DialogContent>
          </Dialog>
